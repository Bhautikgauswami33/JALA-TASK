using System;

namespace CSharpClasses
{
    // Employee class for task 1
    class Employee
    {
        public string name;
        public int empId;
        public string department;

        // Constructor with parameters
        public Employee(string name, int empId, string department)
        {
            this.name = name;
            this.empId = empId;
            this.department = department;
        }

        // Method to print employee details
        public void PrintDetails()
        {
            Console.WriteLine($"Employee Name: {name}");
            Console.WriteLine($"Employee ID: {empId}");
            Console.WriteLine($"Department: {department}");
        }
    }

    // Overloaded constructors example for task 2
    class EmployeeOverloaded
    {
        public string name;
        public int empId;
        public string department;

        // Default constructor
        public EmployeeOverloaded()
        {
            name = "Unknown";
            empId = 0;
            department = "General";
        }

        // Constructor with one parameter
        public EmployeeOverloaded(string name)
        {
            this.name = name;
            empId = 0;
            department = "General";
        }

        // Constructor with two parameters
        public EmployeeOverloaded(string name, int empId)
        {
            this.name = name;
            this.empId = empId;
            department = "General";
        }

        // Constructor with three parameters
        public EmployeeOverloaded(string name, int empId, string department)
        {
            this.name = name;
            this.empId = empId;
            this.department = department;
        }

        public void DisplayDetails()
        {
            Console.WriteLine($"Name: {name}, ID: {empId}, Department: {department}");
        }
    }

    // Car class for tasks 3 and 4
    class Car
    {
        // Three class members (fields)
        public string color;
        public int maxSpeed;
        public string model;

        // Two additional fields
        private string brand;
        private int year;

        // One method
        public void DisplayCarInfo()
        {
            Console.WriteLine($"Car Details - Brand: {brand}, Model: {model}, Color: {color}, Max Speed: {maxSpeed} mph, Year: {year}");
        }

        // Constructor
        public Car(string color, int maxSpeed, string model, string brand, int year)
        {
            this.color = color;
            this.maxSpeed = maxSpeed;
            this.model = model;
            this.brand = brand;
            this.year = year;
        }
    }

    class Classes
    {
        static void Main(string[] args)
        {
            Console.WriteLine("=== C# Classes Assignments ===\n");

            // 1. Construct a class using any methods for employee details using its parameters and print
            Employee emp1 = new Employee("John Doe", 101, "IT");
            emp1.PrintDetails();
            Console.WriteLine();

            // 2. Write a program using overloading class constructor
            EmployeeOverloaded emp2 = new EmployeeOverloaded();
            EmployeeOverloaded emp3 = new EmployeeOverloaded("Jane Smith", 102);
            EmployeeOverloaded emp4 = new EmployeeOverloaded("Bob Johnson", 103, "HR");

            Console.WriteLine("Overloaded constructors:");
            emp2.DisplayDetails();
            emp3.DisplayDetails();
            emp4.DisplayDetails();
            Console.WriteLine();

            // 3 & 4. Create a Car class with members and create an object called myObj
            Car myObj = new Car("Red", 200, "Sedan", "Toyota", 2022);
            Console.WriteLine($"Car color: {myObj.color}");
            Console.WriteLine($"Car maxSpeed: {myObj.maxSpeed}");
            myObj.DisplayCarInfo();
            Console.WriteLine();

            // 5. Write a program on creating multiple objects of one class
            Console.WriteLine("Creating multiple Car objects:");
            Car car1 = new Car("Blue", 180, "Hatchback", "Honda", 2021);
            Car car2 = new Car("Black", 250, "SUV", "BMW", 2023);
            Car car3 = new Car("White", 150, "Sedan", "Hyundai", 2020);

            car1.DisplayCarInfo();
            car2.DisplayCarInfo();
            car3.DisplayCarInfo();

            Console.WriteLine("\nPress any key to exit...");
            Console.ReadKey();
        }
    }
}